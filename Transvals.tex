...
The faces intersecting the spine of such lattices does not always form a semidistributive lattice (consider for example $\mathbf{Jura}_{132}$). However the spine faces of the Tamari lattice form a semidistributive and extremal lattice, which is isomorphic to the lattice of nested topologies.
...
We define $\SpTrvGraph(\Sha)$ as the set of pairs $(x,k)$ with $x\in \Sha$ and $k\in \{0,1\}$, endowed with the binary relation where $(x, 0)\rightarrow_\SpTrvGraph(y,0)$ and $(x, 1)\rightarrow_\SpTrvGraph(y,1)$ if $x\rightarrow y$, and $(x, 0)\rightarrow_\SpTrvGraph(y,1)$ if $x< y$ in $\SpinePos$.
	...
	\begin{table}[H]
		...
		\end{tabular}
		\caption{\centering The number of spine faces, spine intervals and spine transvals of $\Tam_n$.}
	\end{table}
