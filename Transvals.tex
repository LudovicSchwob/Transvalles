...
A lattice respecting the dual condition is called \defn{join-semidistributive}. A lattice which is both meet- and join-distributive is \defn{semidistributive}.
...
\end{prop}
However, $\Fac_\L$ is not a sublattice of $\Int_\L$, and $\SkFac_\L$ is not a sublattice of $\SkInt_\L$.  $\SkFac_\L$ contains another sublattice isomorphic to $\Fac_\L$, consisting of pairs $(y,x)$ such that $[x,y]\in \Fac_\L$.
